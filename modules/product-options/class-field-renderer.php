...
class EzLens_Product_Options_FieldRenderer {
    ...
    public function render_fields() {
        ...
        // اگر محصول متغیر است، شناسه والد را بگیر
        ...
        if (!$template) {
            // برای دیباگ: اگر پالتی نیست، هیچ کاری نکن
            return;
        }
        
        $fields = $template['fields'] ?? [];
        
        // حذف فیلدهای کد (که با _code_ شروع می‌شوند)
        ...
        echo '<span class="price" id="ezlens-total-price">' . wc_price($product->get_price()) . '</span>';
        echo '</div>';
        echo '</div>';
    }

    /**
     * رندر یک فیلد (با پشتیبانی از همه نوع‌ها)
     */
    private function render_field($key, $field) {
        ...
        switch ($type) {
            ...
            case 'upload':
                ...
                echo '>';
                echo '<div class="ezlens-upload-progress"><div class="progress-bar"></div></div>';
    ...
    public function add_cart_item_data($cart_item_data, $product_id, $variation_id) {
        if (isset($_POST['ezlens_options']) && is_array($_POST['ezlens_options'])) {
            ...
            $cart_item_data['ezlens_product_id'] = $product_id; // برای دسترسی در مراحل بعد
        }
    ...
    public function display_cart_item_data($item_data, $cart_item) {
        if (isset($cart_item['ezlens_options']) && is_array($cart_item['ezlens_options'])) {
            ...
            foreach ($cart_item['ezlens_options'] as $key => $value) {
                ...
                if (!empty($value)) {
                    $label = $this->get_field_label($key, $product_id);
                    $item_data[] = [
                        'name' => $label ?: $key,
                        'value' => $value
                    ];
